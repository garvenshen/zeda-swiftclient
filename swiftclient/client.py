...
def get_keystoneclient_2_0bytoken(auth_url, user, token, os_options):
    """
    Authenticate against a auth 2.0 server.
...
def get_auth(auth_url, user, key, **kwargs):
    ...
    #authed_token = kwargs.get('token', None)
    authed_token = os_options.get('auth_token', None)
    ...
    if auth_version in ['2.0', '2', 2]:
        ...
        # add by garvenshen, we allowed authed token to re-auth from keystone
        if authed_token:
            return get_keystoneclient_2_0bytoken(auth_url, user,
                                                   authed_token, 
                                                   os_options)
...
def get_auth_superadmin(connection=None, **args):
    ...
    superadminname = args.get('superadminname', None)
    token = args.get('token', None)
    password = args.get('password', None)
    superadmintenant_name = args.get('superadmintenant_name', None)
    url = None
    os_options = args.get('os_options', {})
    
    if connection:
        url, token = connection.get_auth()
    else:
        if token:
            os_options['auth_token'] = token
        connection = Connection(authurl=ks_authurl,
                        user=superadminname,
                        key=password,
                        tenant_name=superadmintenant_name,
    ...
    from keystoneclient import exceptions
    try:
        _ksclient = ksclient.Client(username=superadminname,
                                    password=password,
                                    token=token,
                                    tenant_name=superadmintenant_name,
                                    auth_url=ks_authurl)
    ...
    tenants = _ksclient.tenants.list()
    superadmintenantid = ''
    for tempheader in tenants:
        if tempheader.name == superadminname:
            superadmintenantid = tempheader.id
        else:
            continue
    endpoint_head = url.split(superadmintenantid)[0]
    return endpoint_head, token, tenants
...
def superadmin_head_container(connection=None, **args):
    ...
    for tes in tenants:
        ...
        if tes.name == tenant_name:
            ...
            ret['tenant_name'] = tes.name
            ret['tenant_id'] = tes.id
            ret['resps'] = head_container(endpointurl, token, container)
            return ret
    return {}
...
def superadmin_head_object(connection=None, **args):
    ...
    for tes in tenants:
        ...
        if tes.name == tenant_name:
            ...
            ret['tenant_name'] = tes.name
            ret['tenant_id'] = tes.id
            ret['resps'] = head_object(endpointurl, token, container, object)
            return ret
    return {}
